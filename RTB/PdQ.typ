...
#figure(
  block(
    ...
    table(
      ...
      [TS-2.1], [Verificare la validazione del formato delle credenziali in fase di login.], [UC2],
      [TS-2.2], [Verificare la segnalazione di errore per identificativo (username) non presente a sistema.], [UC2.0.2],
      [TS-2.3], [Verificare la segnalazione di errore in caso di password non corrispondente all'identificativo fornito.], [UC2.0.2],
      ...
      [TS-3.2], [Verificare la gestione degli errori tecnici durante lo scambio dei parametri di autorizzazione.], [UC3.0.1],
      [TS-3.3], [Verificare l'inibizione del collegamento se il profilo GitHub risulta già associato a un altro utente.], [UC3.0.2],
      [TS-3.4], [Verificare la gestione del mancato consenso dell'utente sulla piattaforma GitHub.], [UC3.0.3],
      ...
      [TS-5], [Verificare la navigazione e la corretta visualizzazione dell'elenco dei repository analizzati.], [UC5],
      [TS-5.1], [Verificare la visualizzazione dell'informativa “lista vuota” in assenza di repository analizzati.], [UC5.0.1],
      [TS-5.2], [Verificare l'inibizione del rendering e la notifica di errore qualora i servizi di persistenza non siano raggiungibili.], [UC5.0.2],
      ...
      [TS-15.5], [Verificare la corretta persistenza e la notifica di successo post-modifica.], [UC15.3],
      ...
      [TS-17.1], [Verificare la gestione dell'errore di comunicazione con GitHub.], [UC17.0.1],
      [TS-17.2], [Verificare che l'Orchestratore tenti l'accesso tramite credenziali o token OAuth in caso di repository privato non raggiungibile pubblicamente.], [UC17.1],
      [TS-17.3], [Verificare che l'Orchestratore annulli l'audit se tutti i metodi di accesso falliscono.], [UC17.1.1],
      ...
      [TS-27], [Verificare l'esposizione delle informazioni identificative del repository selezionato.], [UC27],

      // --- CANCELLAZIONE PROFILO (UC28) ---
      [TS-28], [Verificare la corretta cancellazione del profilo CodeGuardian a seguito della verifica dell'identità tramite password e della conferma definitiva dell'utente.], [UC28, UC28.1],
      [TS-28.1], [Verificare che a seguito della cancellazione vengano rimossi i dati personali e le associazioni OAuth e che le credenziali precedenti risultino invalide.], [UC28.1],
...
#figure(
  block(
    ...
    table(
      ...
      // --- AUTENTICAZIONE (UC2) ---
      [TU-2.1], [FROb25], [Verifica rendering pagina di Login.], [Visualizzazione corretto del form di autenticazione.],
      ...
      [TU-2.13], [FROb36], [Verifica notifica username non esistente nel sistema.], [Feedback specifico per username non censito.],
      [TU-2.14], [FROb36], [Verifica notifica formato password errato in login.], [Feedback su errore sintattico password; procedura inibita.],
      [TU-2.15], [FROb36], [Verifica notifica password errata (hash non corrisponde).], [Feedback specifico per credenziali non corrispondenti.],

      // --- INTEGRAZIONE CON GITHUB (UC3) ---
      [TU-3.1], [FROb37], [Verifica accesso sezione collegamento del profilo GitHub.], [Sezione raggiungibile.],
      [TU-3.2], [FROb38], [Verifica impedimento accesso se GitHub già associato.], [Procedura bloccata se account GitHub già collegato.],
      [TU-3.3], [FROb39], [Verifica utilizzo parametro “state” per prevenzione attacchi CSRF nel flusso OAuth2.], [Stato CSRF generato e verificato; richieste prive di state valido rifiutate.],
      [TU-3.4], [FROb40], [Verifica cifratura Access Token GitHub nel database (AES-256 o equivalente).], [I token GitHub non sono salvati in chiaro; algoritmo di cifratura forte applicato.],
      [TU-3.5], [FROb41], [Verifica assenza di persistenza token in caso di procedura fallita.], [Nessun token o associazione parziale mantenuta in caso di errore.],
      [TU-3.6], [FROb42], [Verifica rendering avviso informativo obbligatorio pre-reindirizzamento su dominio esterno.], [Visualizzazione informativa di sicurezza prima del redirect verso GitHub.],
      [TU-3.7], [FROb43], [Verifica gestione comando annulla redirect con ripristino stato.], [Esecuzione o blocco del redirect; in caso di annullamento la sezione integrazioni viene ripristinata.],
      [TU-3.8], [FROb44], [Verifica elaborazione esito e visualizzazione messaggio al ritorno da GitHub.], [Esito della procedura di associazione mostrato all'utente al rientro su CodeGuardian.],
      [TU-3.9], [FROb44], [Verifica gestione errore sincronizzazione.], [Feedback su fallimento tecnico della procedura; possibilità di ripetere.],
      [TU-3.10], [FROb45], [Verifica blocco associazione per profilo GitHub già in uso su altro account.], [Inibizione collegamento se account GitHub già censito su diverso profilo.],
      [TU-3.11], [FROb46], [Verifica gestione errore “Consenso Negato” su GitHub.], [Feedback specifico per interruzione volontaria dell'utente su piattaforma esterna.],
      ...
      [TU-4.10], [FROb53], [Verifica invio avviso immediato con causa errore in caso di interruzione analisi.], [Notifica di errore con breve spiegazione generata tempestivamente.],

      // --- LISTA REPOSITORY (UC5) ---
      [TU-5.1], [FROb57], [Verifica ordinamento lista repository per data ultima analisi (decrescente).], [Repository visualizzati in ordine dall'analisi più recente.],
      [TU-5.2], [FROb61], [Validazione logica per visualizzazione informativa lista vuota.], [Messaggio informativo mostrato.],
      [TU-5.3], [FROb62], [Verifica inibizione rendering lista e notifica errore in caso di persistenza irraggiungibile.], [Messaggio di errore tecnico mostrato; lista non renderizzata.],
      [TU-5.4], [FROb63], [Verifica presenza e funzionamento comando di aggiornamento (Refresh) in caso di errore.], [Nuovo tentativo di caricamento avviato al click.],
      [TU-5.5], [FROb58], [Verifica consultabilità risultati nell'area personale anche in assenza di notifica email.], [Il report è accessibile dalla dashboard indipendentemente dalla ricezione della notifica.],
      [TU-5.6], [FROb59], [Verifica contrassegno analisi come “Fallita” nella lista progetti utente.], [Stato “Fallita” visibile nella dashboard dell'utente.],
      [TU-5.7], [FROb60], [Verifica visibilità cause del fallimento nella dashboard indipendentemente dalla notifica.], [Dettaglio errore consultabile nella dashboard a prescindere dall'invio dell'avviso.],

      // --- VISUALIZZAZIONE REPORT (UC6) ---
      [TU-6.1], [FROb64], [Verifica selezione e caricamento report da lista.], [Caricamento riuscito dei dati del report selezionato.],
      [TU-6.2], [FROb65], [Verifica validazione lato server: report appartiene al repository dell'utente.], [Rendering inibito con errore di autorizzazione per report non associati al profilo.],
      [TU-6.3], [FROb66], [Verifica inibizione rendering per report non autorizzati.], [Errore di autorizzazione per report non associati al profilo.],
      [TU-6.4], [FROb67], [Verifica gestione timeout nel recupero dati analitici con notifica utente.], [Notifica di indisponibilità temporanea mostrata in caso di timeout.],
      [TU-6.5], [FROb68], [Verifica stato dei filtri aree (toggle on/off).], [Le aree analitiche (Codice, Sicurezza, Doc.) sono correttamente filtrate.],
      [TU-6.6], [FROb69], [Verifica aggiornamento dinamico contenuto in base ai filtri senza ricaricamento pagina.], [Il report si aggiorna alla variazione dei filtri.],
      [TU-6.7], [FROb70], [Controllo validazione: almeno un'area attiva nei filtri.], [Blocco visualizzazione con avviso informativo se nessuna sezione selezionata.],
      [TU-6.8], [FROb71], [Verifica esposizione metadati identificativi del report.], [Metadati caricati correttamente.],
      [TU-6.9], [FROb72], [Verifica correttezza timestamp generazione audit (formato ISO 8601).], [Data e ora corrispondono al record del database.],
      [TU-6.10], [FROb73], [Verifica corrispondenza SHA commit GitHub con link diretto al commit.], [Link verso GitHub funzionante.],
      [TU-6.11], [FROb74], [Controllo visualizzazione username richiedente report.], [Lo username corrisponde all'autore della richiesta di analisi.],
      [TU-6.12], [FROb75], [Verifica integrità metriche tecniche aggregate per aree attive.], [Dati numerici (punteggi, numero bug, vulnerabilità) visualizzati correttamente.],
      [TU-6.13], [FROb76], [Verifica caricamento e visualizzazione lista azioni correttive (remediation).], [Lista remediation caricata correttamente e associata alle criticità rilevate.],
      ...
      // --- METRICHE COMPARATIVE (UC8) ---
      [TU-8.1], [FRDe85], [Verifica logica di generazione dataset per grafici dinamici (linee/ istogrammi).], [Dati trasformati correttamente in serie storiche visualizzabili.],
      [TU-8.2], [FRDe86], [Verifica tooltip informativi all'hover sui punti dati del grafico.], [Valore esatto e hash commit mostrati al passaggio del cursore.],
      [TU-8.3], [FROb87], [Verifica allineamento dati tra vista grafica e tabellare.], [Dati coerenti tra grafico e tabella.],
      [TU-8.4], [FROb88], [Verifica popolamento righe tabella comparativa in ordine cronologico crescente.], [Ordinamento cronologico corretto.],
      [TU-8.5], [FROb89], [Validazione algoritmo calcolo indicatori di variazione (trend incrementali/decrementali).], [Calcolo variazione percentuale tra analisi consecutive eseguito correttamente.],
      [TU-8.6], [FROb90], [Verifica fallback visualizzazione dati grezzi in tabella in caso di errore rendering grafico.], [In assenza di librerie grafiche i dati vengono mostrati in formato tabellare.],

      // --- ANALISI DEL CODICE (UC9) ---
      [TU-9.1], [FROb91], [Verifica caricamento sezione “Codice” solo se area attiva nei filtri.], [Modulo renderizzato con parametri corretti; non caricato se area non attiva.],
      [TU-9.2], [FROb92], [Verifica esposizione risultati analisi statica con gravità e posizione nel file sorgente.], [Bug, code smell e vulnerabilità mostrati con livello di gravità e riferimento al file e riga.],
      [TU-9.3], [FROb93], [Controllo calcolo percentuale copertura test (Code Coverage) e rapporto test superati/falliti.], [Valore normalizzato 0-100% e conteggio test calcolati correttamente.],
      [TU-9.4], [FROb94], [Verifica presentazione lista remediation codice con navigazione al dettaglio.], [Soluzioni visualizzate con link al dettaglio della singola azione correttiva.],
      [TU-9.5], [FROb95], [Controllo informativa “Codice Conforme” in assenza di bug o violazioni qualitative.], [Esito positivo mostrato.],

      // --- ANALISI SICUREZZA (UC10) ---
      [TU-10.1], [FROb96], [Verifica caricamento asincrono sezione sicurezza rispetto alle altre sezioni.], [I dati di sicurezza vengono caricati in modo indipendente senza bloccare le altre aree.],
      [TU-10.2], [FROb97], [Verifica esposizione dipendenze vulnerabili con codice CVE, severità CVSS e versione sicura.], [Librerie obsolete o vulnerabili elencate con tutti i campi informativi richiesti.],
      ...
      [TU-11.3], [FROb103], [Calcolo dell'indice di completezza documentale basato sulla copertura delle interfacce pubbliche.], [Rapporto coerente tra interfacce pubbliche del codice e blocchi di documentazione presenti.],
      [TU-11.4], [FROb104], [Verifica esposizione suggerimenti testuali per integrazione documentazione mancante.], [Suggerimenti di miglioramento visualizzati correttamente per ogni lacuna rilevata.],
      [TU-11.5], [FROb105], [Controllo informativa “Documentazione Completa” in assenza di criticità documentali.], [Esito positivo mostrato se non ci sono errori o mancanze documentali rilevate.],

      // --- RANKING (UC12) ---
      [TU-12.1], [FROb106], [Verifica del calcolo del punteggio di qualità globale (0-100) basato su metriche pesate.], [Il punteggio finale è la media pesata dei punteggi di codice, sicurezza e documentazione.],
      [TU-12.2], [FROb107], [Algoritmo di generazione graduatoria con ordinamento decrescente per punteggio.], [La lista viene ordinata correttamente dal punteggio più alto al più basso.],
      [TU-12.3], [FROb108], [Verifica esposizione dati per ogni riga: posizione, nome repository, punteggio, trend mensile.], [Tutti i campi previsti per ogni riga del ranking vengono popolati correttamente.],
      [TU-12.4], [FROb109], [Verifica inibizione rendering ranking e informativa in assenza di analisi completate.], [Messaggio che suggerisce la prima analisi mostrato se il ranking è vuoto.],

      // --- DISCONNESSIONE GITHUB (UC13) ---
      [TU-13.1], [FROb110], [Verifica richiesta conferma esplicita prima della rimozione integrazione GitHub.], [Operazione bloccata in assenza di conferma; dialog di conferma presente.],
      [TU-13.2], [FROb111], [Verifica della funzione di revoca token OAuth verso le API di GitHub.], [Richiesta di revoca inviata correttamente alle API GitHub al momento della conferma.],
      [TU-13.3], [FROb112], [Verifica eliminazione definitiva di token (access/refresh) e ID GitHub dal database.], [Token e ID utente GitHub rimossi dal DB dopo la disconnessione.],
      [TU-13.4], [FROb113], [Verifica gestione errore comunicazione con GitHub durante revoca: pulizia locale garantita.], [Anche in caso di errore API GitHub i dati locali sensibili vengono eliminati.],

      // --- ESPORTAZIONE (UC14) ---
      [TU-14.1], [FRDe113], [Verifica disponibilità link di download del file generato.], [Link di download presente e funzionante dopo la generazione del file.],
      [TU-14.2], [FRDe114], [Verifica supporto formati di esportazione PDF e JSON.], [Accetta esclusivamente PDF o JSON come formati validi.],
      [TU-14.3], [FRDe115], [Verifica inibizione invio richiesta in assenza di formato selezionato.], [Messaggio di errore e blocco se l'utente tenta di esportare senza scegliere il formato.],
      [TU-14.4], [FRDe116], [Verifica del modulo di generazione file: mapping dati e inclusione metadati.], [I dati del report, inclusi metadati e sezioni analizzate, vengono mappati senza perdite.],
      [TU-14.5], [FRDe117], [Verifica processo di generazione file asincrono senza blocco dell'interfaccia.], [L'UI rimane responsiva durante il parsing di report voluminosi.],

      // --- MODIFICA PASSWORD (UC15) ---
      [TU-15.1], [FROb118], [Verifica accesso alla sezione dedicata alla modifica password.], [Rendering corretto del modulo di modifica password nell'area profilo.],
      [TU-15.2], [FROb119], [Confronto hash tra password corrente inserita e hash memorizzato nel database.], [Restituisce true solo se i due hash corrispondono.],
      [TU-15.3], [FROb120], [Verifica inibizione procedura e notifica errore per password corrente mancante o errata.], [Messaggio specifico mostrato sia per campo vuoto sia per password non corrispondente.],
      [TU-15.4], [FROb121], [Validatore criteri complessità nuova password.], [Rigetto di password che non rispettano i requisiti di lunghezza e complessità.],
      [TU-15.5], [FROb122], [Controllo eterogeneità: nuova password diversa da quella attuale.], [Restituisce errore se l'hash della nuova password coincide con quello attuale.],
      [TU-15.6], [FROb123], [Verifica aggiornamento password nella persistenza tramite nuovo hashing sicuro e salt univoco.], [La nuova password viene salvata cifrata con algoritmo sicuro e salt rigenerato.],
      [TU-15.7], [FROb124], [Verifica invio notifica email automatica a seguito di modifica password avvenuta.], [Email di notifica inviata all'indirizzo associato al profilo dopo il cambio.],
      [TU-15.8], [FROb125], [Verifica invalidazione di tutte le sessioni attive post-cambio password.], [Le sessioni parallele risultano invalidate; sessione corrente mantenuta attiva.],

      // --- VISUALIZZAZIONE REMEDIATION GENERICA (UC16) ---
      [TU-16.1], [FROb126], [Verifica visualizzazione dettaglio tecnico singola remediation selezionata.], [Dettaglio correttamente caricato e visualizzato per la remediation selezionata.],
      [TU-16.2], [FROb127], [Verifica esposizione campi obbligatori remediation: descrizione difetto, snippet, severità, proposta.], [Tutti i campi previsti presenti.],
      [TU-16.3], [FROb128], [Verifica inclusione riferimenti esterni per vulnerabilità di sicurezza note.], [Link a documentazione esterna mostrati correttamente per remediation di tipo sicurezza.],

      // --- VERIFICA ACCESSIBILITA' REPOSITORY (UC17) ---
      [TU-17.1], [FROb129], [Verifica gestione ciclo di vita verifica accessibilità tramite chiamate asincrone API GitHub.], [Chiamate asincrone eseguite correttamente; stato della verifica tracciato.],
      [TU-17.2], [FROb130], [Verifica meccanismo “Exponential Backoff” per tentativi di riconnessione su errori di rete.], [Ritardi crescenti tra tentativi; interruzione dopo il numero massimo configurato.],
      [TU-17.3], [FROb131], [Verifica validazione raggiungibilità endpoint GitHub tramite richiesta “Heartbeat”.], [Heartbeat inviato prima del fetch; verifica operatività del servizio remoto.],
      [TU-17.4], [FROb132], [Verifica tentativo accesso pubblico al repository prima dell'uso di credenziali.], [Prima richiesta senza intestazioni di autorizzazione.],
      [TU-17.5], [FROb133], [Verifica accesso privato via token OAuth/PAT su errore HTTP 403/404 della risorsa pubblica.], [Seconda richiesta con token utente iniettato a seguito di errore 403/404.],
      [TU-17.6], [FROb134], [Verifica controllo “scopes” del token: permessi minimi di lettura (repo / public_repo).], [Token con scopes insufficienti viene rifiutato con errore specifico.],
      [TU-17.7], [FROb135], [Verifica aggiornamento stato analisi a “FAILED_ACCESS” e segnale interruzione in caso di fallimento definitivo.], [Stato impostato a “FAILED_ACCESS”; modulo notifica informato del fallimento.],

      // --- ACCETTAZIONE/RIFIUTO REMEDIATION GENERICA (UC18, UC19) ---
      [TU-18.1], [FRDe136], [Verifica applicazione automatica modifiche al repository tramite integrazione GitHub.], [Commit con le modifiche inviato correttamente al repository remoto.],
      [TU-18.2], [FRDe137], [Verifica validazione di integrità della proposta correttiva prima del commit.], [Proposta validata prima dell'invio; commit non eseguito se la validazione fallisce.],
      [TU-18.3], [FRDe138], [Verifica aggiornamento stato remediation in “Applied” o “Dismissed” nel database.], [Stato correttamente aggiornato a seguito dell'azione dell'utente.],
      [TU-18.4], [FRDe139], [Verifica notifica all'utente in caso di fallimento del commit sul repository remoto.], [Messaggio di errore mostrato; codebase rimane invariata.],

      // --- CREAZIONE RACCOLTA REPORT (UC20) ---
      [TU-20.1], [FROb140], [Verifica definizione nome univoco raccolta.], [Nomi non conformi ai vincoli rifiutati con messaggio specifico.],
      [TU-20.2], [FROb141], [Verifica validazione sintattica URL GitHub: HTTPS + struttura path user/repo.], [URL non conformi rifiutati con avviso circa il protocollo o il formato richiesto.],
      [TU-20.3], [FROb142], [Verifica interrogazione API GitHub per conferma esistenza e raggiungibilità repository.], [Repository inesistenti o irraggiungibili bloccano la creazione della raccolta.],
      [TU-20.4], [FROb143], [Verifica gestione repository inaccessibile (privato senza permessi) con notifica utente.], [Avviso specifico per repository privato non accessibile dall'utente.],
      [TU-20.5], [FROb144], [Verifica impedimento creazione raccolta duplicata per stesso repository e stesso utente.], [Errore di duplicazione se l'URL è già associato a una raccolta dell'utente.],
      [TU-20.6], [FROb145], [Verifica memorizzazione descrizione facoltativa con supporto codifica UTF-8.], [Caratteri speciali e simboli memorizzati correttamente.],

      // --- AVVIO ANALISI (UC21) ---
      [TU-21.1], [FROb146], [Verifica parallelizzazione richieste di analisi verso i diversi strumenti esterni.], [Le richieste agli strumenti vengono inviate in parallelo.],
      [TU-21.2], [FROb147], [Verifica inclusione parametri di configurazione utente nelle richieste agli strumenti.], [Parametri definiti in fase di richiesta correttamente trasmessi agli strumenti esterni.],
      [TU-21.3], [FROb148], [Verifica trasmissione sicura delle credenziali al servizio AWS per la clonazione.], [Credenziali non esposte in chiaro; passate tramite secret manager.],
      [TU-21.4], [FROb149], [Verifica monitoraggio completamento clonazione e gestione timeout / disco insufficiente.], [Timeout e errori di spazio rilevati; procedura interrotta con segnalazione.],
      [TU-21.5], [FROb150], [Verifica inibizione invio richieste agli strumenti e liberazione risorse in caso di errore clonazione.], [Nessuna richiesta inoltrata agli strumenti.],
      [TU-21.6], [FROb151], [Verifica inoltro codebase agli strumenti di analisi via protocolli sicuri.], [File trasmessi agli strumenti tramite canali cifrati.],

      // --- PERSISTENZA STATO ANALISI (UC22) ---
      [TU-22.1], [FROb152], [Verifica registrazione stato analisi come “PENDING” a seguito di inizializzazione strumenti.], [Stato “PENDING” correttamente scritto nel database dopo l'avvio degli strumenti.],
      [TU-22.2], [FROb153], [Verifica associazione univoca ID analisi a repository e utente richiedente.], [ID analisi associato correttamente; impossibile creare duplicati per stessa coppia repository-utente.],
      [TU-22.3], [FROb154], [Verifica persistenza metadati di avvio: hash commit analizzato e timestamp di sistema.], [Hash commit e timestamp registrati correttamente al momento dell'avvio.],
      [TU-22.4], [FROb155], [Verifica procedura di Rollback e segnalazione agli strumenti in caso di errore scrittura stato.], [In caso di fallimento persistenza gli strumenti vengono informati di annullare l'analisi.],
      [TU-22.5], [FROb156], [Verifica registrazione nei log di audit di ogni fallimento di persistenza con stack trace.], [Log di errore scritti con dettaglio diagnostico completo.],

      // --- RECUPERO RISULTATI (UC23) ---
      [TU-23.1], [FROb157], [Verifica polling periodico o ricezione segnale di completamento dagli strumenti esterni.], [L'orchestratore rileva correttamente la disponibilità dei risultati.],
      [TU-23.2], [FROb158], [Verifica download dei risultati non appena disponibili dagli strumenti.], [File dei risultati scaricati correttamente all'atto della disponibilità.],
      [TU-23.3], [FROb159], [Verifica controllo integrità e leggibilità dei file ricevuti.], [File corrotti o incompleti rilevati; segnalazione prima dell'utilizzo.],
      [TU-23.4], [FROb160], [Verifica prosecuzione creazione report con dati parziali in caso di fallimento di uno strumento.], [Report generato con i dati disponibili; sezione mancante segnalata come assente.],
      [TU-23.5], [FROb161], [Verifica impostazione timeout massimo di attesa per strumento ritardatario.], [Attesa interrotta dopo il timeout configurato; prosecuzione con dati disponibili.],
      [TU-23.6], [FROb162], [Verifica segnalazione nel database di report contenente dati parziali.], [Flag “parziale” impostato correttamente nel record del report nel DB.],

      // --- GENERAZIONE REPORT FINALE (UC24) ---
      [TU-24.1], [FROb163], [Verifica unificazione dati provenienti dai diversi strumenti in documento di sintesi unico.], [Output di codice, sicurezza e documentazione aggregati correttamente in un unico report.],
      [TU-24.2], [FROb164], [Verifica conversione formati eterogenei degli strumenti in modello standard comune.], [Tutti i formati raw vengono normalizzati correttamente senza perdita di informazioni.],
      [TU-24.3], [FROb165], [Verifica validazione completezza report prima del salvataggio (risultati, data, versione codice).], [Report privo di informazioni essenziali non viene inoltrato alla fase di salvataggio.],
      [TU-24.4], [FROb166], [Verifica calcolo punteggi di riepilogo basati sui risultati delle singole aree analizzate.], [Punteggi aggregati calcolati correttamente e coerenti con i risultati delle singole sezioni.],

      // --- SALVATAGGIO REPORT (UC25) ---
      [TU-25.1], [FROb167], [Verifica archiviazione permanente report con collegamento a repository e utente.], [Report salvato correttamente e associato al repository e al profilo richiedente.],
      [TU-25.2], [FROb168], [Verifica aggiornamento stato analisi a “Completato” solo dopo conferma salvataggio riuscito.], [Stato impostato a “Completato” esclusivamente se la scrittura nel DB ha avuto esito positivo.],
      [TU-25.3], [FROb169], [Verifica notifica utente con messaggio di errore in caso di impossibilità di salvataggio.], [Messaggio di errore generato e reso disponibile per la notifica all'utente.],
      [TU-25.4], [FROb170], [Verifica tracciamento interno dei motivi di fallimento del salvataggio per audit tecnico.], [Log di errore dettagliato scritto con causa del fallimento.],
      [TU-25.5], [FROb171], [Verifica mantenimento copia temporanea del report in caso di errore salvataggio definitivo.], [Copia temporanea disponibile per recupero manuale o retry automatico.],

      // --- NOTIFICA COMPLETAMENTO (UC26) ---
      [TU-26.1], [FROb172], [Verifica generazione automatica notifica utente a report pronto e salvato.], [Notifica generata automaticamente non appena il report risulta archiviato.],
      [TU-26.2], [FROb173], [Verifica inclusione link/pulsante accesso diretto al report nella notifica.], [Notifica contiene riferimento diretto al report consultabile.],
      [TU-26.3], [FROb174], [Verifica inclusione informazioni identificative nella notifica (nome repository, data).], [Nome repository e data di esecuzione presenti nel corpo della notifica.],
      [TU-26.4], [FROb175], [Verifica indipendenza stato report dall'esito dell'invio notifica.], [Report rimane disponibile anche se la notifica fallisce.],
      [TU-26.5], [FROb176], [Verifica registrazione anomalia nei log interni in caso di errore invio notifica.], [Log di errore scritto con dettaglio del canale e del tipo di anomalia.],
      [TU-26.6], [FROb177], [Verifica meccanismo di retry invio notifica per problemi temporanei di rete.], [Numero configurato di tentativi effettuati prima di rinunciare; log aggiornato.],

      // --- VISUALIZZAZIONE INFORMAZIONI REPOSITORY (UC27) ---
      [TU-27.1], [FROb179], [Controllo mapping metadati: Nome, URL, Data ultima analisi per ogni elemento.], [Oggetti lista popolati correttamente con tutti i campi richiesti.],

      // --- CANCELLAZIONE PROFILO (UC28) ---
      [TU-28.1], [FROb180], [Verifica richiesta inserimento password attuale come verifica identità obbligatoria pre-cancellazione.], [Procedura di cancellazione bloccata senza verifica password corretta.],
      [TU-28.2], [FROb181], [Verifica visualizzazione avviso di irreversibilità con possibilità di annullamento.], [Dialog di avviso irreversibilità mostrato; annullamento disponibile.],
      [TU-28.3], [FROb182], [Verifica rimozione dati personali, associazioni OAuth e invalidazione credenziali post-cancellazione.], [Accesso con credenziali precedenti impossibile.],

      // --- GESTIONE OAUTH GITHUB (UC29) ---
      [TU-29.1], [FROb183], [Verifica scambio codice temporaneo OAuth con token di accesso permanente.], [Richiesta di scambio correttamente inoltrata a GitHub; token ricevuto e disponibile.],
      [TU-29.2], [FROb184], [Verifica cifratura token GitHub prima del salvataggio nel database.], [Token cifrato con algoritmo forte; non leggibile in chiaro nel DB.],
      [TU-29.3], [FROb185], [Verifica collegamento esclusivo token GitHub al profilo dell'utente autorizzante.], [Token associato univocamente all'account CodeGuardian corretto.],
      [TU-29.4], [FROb186], [Verifica annullamento collegamento e richiesta ripetizione procedura per codice scaduto/invalido.], [Errore di protocollo gestito; utente informato della necessità di riautenticarsi.],

      // --- VISUALIZZAZIONE REMEDIATION CODICE (UC30) ---
      [TU-30.1], [FROb187], [Verifica caricamento e visualizzazione dettaglio singola remediation area codice.], [Dettaglio correttamente caricato per remediation selezionata dall'area codice.],
      [TU-30.2], [FROb188], [Verifica presenza campi obbligatori nella remediation codice.], [Tutti i campi previsti presenti nel pannello di dettaglio.],

      // --- VISUALIZZAZIONE REMEDIATION SICUREZZA (UC31) ---
      [TU-31.1], [FROb189], [Verifica caricamento e visualizzazione dettaglio singola remediation area sicurezza.], [Dettaglio correttamente caricato per remediation selezionata dall'area sicurezza.],
      [TU-31.2], [FROb190], [Verifica presenza campi obbligatori nella remediation sicurezza.], [Tutti i campi previsti presenti nel pannello di dettaglio.],

      // --- VISUALIZZAZIONE REMEDIATION DOCUMENTAZIONE (UC32) ---
      [TU-32.1], [FROb191], [Verifica caricamento e visualizzazione dettaglio singola remediation area documentazione.], [Dettaglio correttamente caricato per remediation selezionata dall'area documentazione.],
      [TU-32.2], [FROb192], [Verifica presenza campi obbligatori nella remediation documentale.], [Tutti i campi previsti presenti nel pannello di dettaglio.],

      // --- ACCETTAZIONE REMEDIATION CODICE (UC33) ---
      [TU-33.1], [FROb193], [Verifica abilitazione comando di accettazione remediation codice per Utente Avanzato.], [Controllo accesso: solo Utente Avanzato vede il comando di accettazione.],
      [TU-33.2], [FROb194], [Verifica applicazione automatica modifiche alla codebase a seguito di accettazione.], [Commit con le modifiche suggerite inviato correttamente al repository.],
      [TU-33.3], [FROb195], [Verifica aggiornamento stato remediation codice a “eseguita” nel database.], [Stato aggiornato correttamente nella dashboard dopo applicazione riuscita.],
      [TU-33.4], [FROb196], [Verifica notifica fallimento e invarianza codebase in caso di errore applicazione remediation codice.], [Messaggio di errore mostrato; nessuna modifica al repository in caso di fallimento.],

      // --- RIFIUTO REMEDIATION CODICE (UC34) ---
      [TU-34.1], [FROb197], [Verifica abilitazione comando di rifiuto remediation codice.], [Comando di rifiuto disponibile nell'interfaccia di dettaglio della remediation.],
      [TU-34.2], [FROb198], [Verifica aggiornamento stato remediation codice a “rifiutata” senza modifiche al repository.], [Stato “rifiutata” aggiornato; codebase rimane invariata.],

      // --- ACCETTAZIONE REMEDIATION SICUREZZA (UC35) ---
      [TU-35.1], [FROb199], [Verifica abilitazione comando di accettazione remediation sicurezza.], [Comando disponibile per Utente Avanzato nell'area sicurezza.],
      [TU-35.2], [FROb200], [Verifica applicazione patch/ configurazioni di sicurezza a seguito di accettazione.], [Modifiche di sicurezza applicate correttamente al repository.],
      [TU-35.3], [FROb201], [Verifica aggiornamento stato remediation sicurezza a “eseguita”.], [Stato aggiornato nella dashboard dopo applicazione riuscita.],
      [TU-35.4], [FROb202], [Verifica notifica fallimento applicazione remediation sicurezza.], [Messaggio di errore e vulnerabilità non mitigata in caso di insuccesso.],

      // --- RIFIUTO REMEDIATION SICUREZZA (UC36) ---
      [TU-36.1], [FROb203], [Verifica abilitazione comando di rifiuto remediation sicurezza.], [Comando di rifiuto disponibile nell'area sicurezza.],
      [TU-36.2], [FROb204], [Verifica aggiornamento stato remediation sicurezza a “rifiutata” senza modifiche al repository.], [Stato aggiornato; repository invariato rispetto alla vulnerabilità segnalata.],

      // --- ACCETTAZIONE REMEDIATION DOCUMENTAZIONE (UC37) ---
      [TU-37.1], [FROb205], [Verifica abilitazione comando di accettazione remediation documentale.], [Comando disponibile per Utente Avanzato nell'area documentazione.],
      [TU-37.2], [FROb206], [Verifica applicazione automatica modifiche ai file documentali a seguito di accettazione.], [File di documentazione aggiornati correttamente nel repository.],
      [TU-37.3], [FROb207], [Verifica aggiornamento stato remediation documentale a “eseguita”.], [Stato aggiornato nella dashboard dopo applicazione riuscita.],
      [TU-37.4], [FROb208], [Verifica notifica errore e invarianza documentazione in caso di fallimento I/O o permessi.], [Messaggio di errore mostrato; file di documentazione rimangono invariati.],

      // --- RIFIUTO REMEDIATION DOCUMENTAZIONE (UC38) ---
      [TU-38.1], [FROb209], [Verifica accesso al dettaglio singola remediation documentale per la procedura di rifiuto.], [Dettaglio remediation visualizzato correttamente prima del rifiuto.],
      [TU-38.2], [FROb210], [Verifica abilitazione comando di rifiuto remediation documentale.], [Comando di rifiuto disponibile nell'area documentazione.],
      [TU-38.3], [FROb211], [Verifica aggiornamento stato remediation documentale a “rifiutata” nel database.], [Stato aggiornato correttamente a seguito del rifiuto confermato.],
      [TU-38.4], [FROb212], [Verifica che il rifiuto non comporti modifiche ai file sorgente o documentali.], [Nessuna modifica al repository dopo il rifiuto.],
      [TU-38.5], [FROb213], [Verifica rimozione o marcatura visiva remediation rifiutata dalla lista azioni pendenti.], [Remediation non più presente come pendente o visivamente segnata come scartata.],
      [TU-38.6], [FROb214], [Verifica visualizzazione conferma visiva avvenuto rifiuto della proposta correttiva.], [Messaggio o badge di conferma rifiuto mostrato all'utente.],

      // --- ANALISI REPOSITORY PRIVATI (UC39) ---
      [TU-39.1], [FROb215], [Verifica abilitazione richiesta analisi repository privato solo per Utente Avanzato con integrazione GitHub attiva.], [Funzionalità disponibile solo se GitHub è collegato; inibita per utenti senza integrazione.],
      [TU-39.2], [FROb216], [Verifica validazione presenza integrazione GitHub valida prima di accettare richiesta per risorsa privata.], [Richiesta bloccata se il token GitHub risulta assente o scaduto.],
      [TU-39.3], [FROb217], [Verifica inibizione richiesta analisi privata in assenza di selezione aree di interesse.], [Errore mostrato se nessuna area selezionata.],

      // --- CATALOGO REPOSITORY PRIVATI (UC40, UC41, UC42) ---
      [TU-40.1], [FROb218], [Verifica inserimento URL repository privato di proprietà nel catalogo personale.], [Repository aggiunto correttamente al catalogo; disponibile per analisi future.],
      [TU-40.2], [FROb219], [Verifica impedimento inserimento URL duplicato nel catalogo personale con notifica.], [Avviso di duplicazione mostrato; catalogo rimane invariato.],
      [TU-41.1], [FROb220], [Verifica ordinamento elenco repository privati registrati in ordine decrescente.], [Lista ordinata per data inserimento.],
      [TU-41.2], [FROb221], [Verifica visualizzazione informativa catalogo vuoto con suggerimento inserimento.], [Messaggio che suggerisce l'inserimento del primo repository mostrato se catalogo vuoto.],
