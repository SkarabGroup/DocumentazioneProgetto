...
\hline
2025-11-25 & 1.0.0 & Prima stesura del documento & Suar Alberto & <<<VERIFICATORE>>> \\
\hline
\end{tabular}
\end{center}
\newpage
...
\newcommand{\Location}{Sede Var Group SpA}
...
\subsection{Apertura}
La riunione, svolta nella \Location, ha avuto inizio alle \MeetingStartingTime, con i referenti dell'azienda che hanno esposto 
...
Prima dell'inizio effettivo della sessione di design thinking, l'azienda proponente ha chiesto di definire, in linea generale, lo scopo del progetto \textit{CodeGuardian}, così da chiarire eventuali dubbi relativi alla presentazione del capitolato e fornire una descrizione chiara di ciò che viene richiesto.

\subsubsection*{Decisione}
Il team ha discusso e condiviso le proprie opinioni riguardo allo scopo del progetto \textit{CodeGuardian}, giungendo alla conclusione che esso consista in \textbf{una piattaforma basata su un sistema ad agenti, capace di analizzare repository e fornire report dettagliati} relativi a controlli del codice — come analisi statica e copertura dei test — \textbf{controlli di sicurezza}, come la verifica del rispetto degli standard OWASP, e \textbf{controlli sulla documentazione}, verificando la presenza di documenti chiari e completi che descrivano accuratamente le funzionalità del prodotto analizzato.
...
\subsubsection*{Sintesi}
Sulla base delle Personas individuate come principali utilizzatrici del prodotto, l'azienda ha richiesto di focalizzarsi sulla definizione delle funzionalità che ciascuna figura professionale si aspetterebbe di trovare in \textit{CodeGuardian}.
...
\begin{itemize}
    \item I tempi di risposta devono essere ragionevoli e, qualora si verifichino problemi durante l'elaborazione, l'utente deve essere avvisato tempestivamente, possibilmente con suggerimenti su come procedere.
    \item Deve essere mostrato l'esito dei test eseguiti, evidenziando quelli eventualmente falliti.
    \item Devono essere forniti, tramite dashboard e rappresentazioni grafiche, indicatori che mostrino il miglioramento (o il peggioramento) rispetto alle versioni precedenti, specificando inoltre quali best practices non sono state rispettate nello sviluppo delle nuove funzionalità.
\end{itemize}
...
\subsubsection*{Decisione}
Il team ha deciso di adottare un approccio basato sull'\textbf{Orchestratore Autocratico}. In questo modello è presente un agente principale (leader) che coordina il lavoro degli altri agenti: definisce i compiti che ciascun agente deve svolgere, fornisce gli strumenti necessari alla loro esecuzione e raccoglie i risultati prodotti, decidendo di volta in volta quale agente attivare successivamente.
\\
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Analisi statica} (eventualmente tramite tool esterni)  
    ...
    \begin{itemize}
        ...
        \item \textbf{Analisi del rispetto delle best practices} (coerenza tra ciò che la documentazione dichiara e ciò che il codice effettivamente realizza)  
    \end{itemize}
    ...
    Nel caso siano individuati errori, vulnerabilità o pratiche scorrette, l’agente di *Remediation* suggerisce possibili soluzioni, fix o azioni correttive.
...
Il diagramma costruito rappresenta quindi una prima versione dell’architettura operativa di *CodeGuardian*, sulla quale il team potrà iterare nelle sessioni successive di progettazione.

                   


\section{Azioni e responsabilità}
In concordanza con l'azienda, verrà svolta una riunione di allineamento \textbf{ogni 2 settimane} (salvo diversa indicazione) al fine di mostrare lo stato di avanzamento del progetto, chiarire problematiche bloccanti e fornire idee strutturate di progettazione. 
